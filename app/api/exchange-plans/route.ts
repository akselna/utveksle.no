...
export async function PATCH(request: Request) {
  try {
    ...
    // If selected_courses is provided, update the saved_courses table
    if (selected_courses !== undefined && Array.isArray(selected_courses)) {
      // First, delete existing courses for this plan
      await query(
        `DELETE FROM saved_courses WHERE exchange_plan_id = $1`,
        [planId]
      );
