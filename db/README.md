...
- `GET /api/reviews?university_id=1` - Hent erfaringer for universitet
- `POST /api/reviews` - Opprett erfaring (krever autentisering)
...
// Hent universiteter
const result = await query("SELECT * FROM universities WHERE country = $1", [
  "Norway",
]);
