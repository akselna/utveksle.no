...
-- This will move all data from duplicate universities to the correct ones, then delete the duplicates
...
-- Also delete these other duplicates with proper cities that aren't in use:
DELETE FROM universities WHERE id IN (45, 71) AND id NOT IN (
  SELECT DISTINCT university_id FROM courses WHERE university_id IS NOT NULL
  UNION
  SELECT DISTINCT university_id FROM exchange_agreements WHERE university_id IS NOT NULL
  UNION
  SELECT DISTINCT university_id FROM exchange_plans WHERE university_id IS NOT NULL
  UNION
  SELECT DISTINCT university_id FROM experiences WHERE university_id IS NOT NULL
);
