  ...
  return (
    <div className="relative flex items-center">
      <button
        ...
        className="relative p-2 text-gray-600 hover:text-gray-900 transition-colors rounded-full hover:bg-gray-100 flex items-center justify-center"
      >
        <Bell size={20} />
      ...
      {isOpen && (
        <div className="fixed md:absolute md:right-0 right-4 left-4 md:left-auto top-14 md:top-full md:mt-2 w-auto md:w-80 max-w-[calc(100vw-2rem)] md:max-w-none bg-white rounded-xl shadow-2xl border border-gray-100 z-[60] overflow-hidden animate-in fade-in slide-in-from-top-2">
            ...
            {notifications.length === 0 ? (
              ...
              <div className="divide-y divide-gray-50">
                {notifications.map(notif => (
                  <div key={notif.id} className={`p-4 transition-colors ${notif.type === 'accepted' ? 'bg-green-50/30 hover:bg-green-50/50' : 'hover:bg-gray-50'}`}>
                    <div className="flex gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${notif.type === 'accepted' ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-900'}`}>
                          ...
                          <>
                            ...
                            <div className="flex gap-2 mt-3">
                              <button
                                ...
                                disabled={loading}
                                className="flex-1 bg-primary text-white text-xs font-bold py-2 rounded-lg hover:bg-primary-hover transition-colors flex items-center justify-center gap-1"
                              >
                                <Check size={14} /> Godta
                              </button>
                              <button
